...
$(function() {
  ...
  function handlebarsCompile() {
    ...
    handlebarsTemplate = Handlebars.compile(main);
  }

  function renderDomTree() {
    var html = handlebarsTemplate(bodyNodeTree);
    $('.dom-tree').html(html);
  }

  function addHandlers() {
    // Drag start event
    $(".components-holder .component").on('dragstart', function(event) {
    ...
    $(".components-holder .component").on('dragend', function() {
      ...
      if (clientFrameWindow) {
        DragDropFunctions.processNodeTree(clientFrameWindow.document.documentElement)
          .then(function(nodeTree) {
            return DragDropFunctions.getNodeTreeFromBody(nodeTree);
          })
          .then(function(results) {
            bodyNodeTree = results;
            renderDomTree();
            console.log(bodyNodeTree);
          })
    ...
    $('#clientframe').on('load', function() {
      // Get iframe content
      clientFrameWindow = $('#clientframe').get(0).contentWindow;
  ...
  var DragDropFunctions = {
    ...
    getNodeTreeFromBody: function(dom) {
      return new Promise(function(resolve) {
        // Find body
        var body = _.find(dom.children, { nodeName: 'BODY' });
        resolve(body);
      });
    }
  };
